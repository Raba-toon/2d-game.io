  ...
  socket.onopen = () => {
    ...
    if (savedPlayerInfo) {
      try {
        ...
        // Réutiliser les identifiants sauvegardés pour se reconnecter automatiquement
  ...
  socket.onmessage = (event) => {
    ...
    if (data.type === "login_response" || data.type === "reconnect_response") {
      if (data.success) {
        ...
        // Ici vous pourriez appeler une fonction pour démarrer le jeu
        // startGame();
      } else {
    ...
    // Traiter les positions des joueurs
    if (data.type === "positions") {
      // Code pour mettre à jour les positions des joueurs à l'écran
...
// Se connecter au serveur WebSocket au chargement de la page
